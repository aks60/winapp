...
public class AreaSimple extends ElemBase {
     ...
     */
//    public AreaSimple(IWindows iwin, AreaSimple root, AreaSimple owner, String id, eLayoutArea layout, float width, float height) {
...
//        if (owner != null && eTypeElem.ARCH == owner.getTypeElem()
//                && owner.getChildList().size() == 2 && eTypeElem.IMPOST == owner.getChildList().get(1).getTypeElem()) {
...
//    }
     ...
     */
    public AreaSimple(AreaSimple owner, String id, eLayoutArea layout, float width, float height, int colorBase, int colorInternal, int colorExternal) {
        super(id);
        this.owner = owner;
        this.layout = layout;
        this.width = width;
...
//    }
    ...
    public void passJoinArea(HashMap<String, ElemJoinig> hmJoin) {
       ...
       /* if(getAdjoinedElem(eLayoutArea.TOP) instanceof AreaSimple || getAdjoinedElem(eLayoutArea.LEFT) instanceof AreaSimple) {
...
//
//        //if (getAdjoinedElem(eLayoutArea.TOP) instanceof AreaSimple) {
//        elemJoinVal = hmJoin.get(key1);
//        if (elemJoinVal == null) {
//            hmJoin.put(key1, new ElemJoinig(getConst()));
//            elemJoinVal = hmJoin.get(key1);
//        }
//        if (elemJoinVal.elemJoinRight == null) elemJoinVal.elemJoinRight = getAdjoinedElem(eLayoutArea.TOP);
//        if (elemJoinVal.elemJoinBottom == null) elemJoinVal.elemJoinBottom = getAdjoinedElem(eLayoutArea.LEFT);
//
...
//            elemJoinVal = hmJoin.get(key2);
//        }
//        if (elemJoinVal.elemJoinLeft == null) elemJoinVal.elemJoinLeft = getAdjoinedElem(eLayoutArea.TOP);
//        if (elemJoinVal.elemJoinBottom == null) elemJoinVal.elemJoinBottom = getAdjoinedElem(eLayoutArea.RIGHT);
//        //}
...
//            elemJoinVal = hmJoin.get(key3);
//        }
//        if (elemJoinVal.elemJoinTop == null) elemJoinVal.elemJoinTop = getAdjoinedElem(eLayoutArea.RIGHT);
//        if (elemJoinVal.elemJoinLeft == null) elemJoinVal.elemJoinLeft = getAdjoinedElem(eLayoutArea.BOTTOM);
//
...
//            elemJoinVal = hmJoin.get(key4);
//        }
//        if (elemJoinVal.elemJoinTop == null) elemJoinVal.elemJoinTop = getAdjoinedElem(eLayoutArea.LEFT);
//        if (elemJoinVal.elemJoinRight == null) elemJoinVal.elemJoinRight = getAdjoinedElem(eLayoutArea.BOTTOM);
    }
    ...
    public <E> LinkedList<E> getElemList(eTypeElem... type) {
        if (type == null) type = new eTypeElem[]{eTypeElem.FRAME, eTypeElem.IMPOST, eTypeElem.GLASS, eTypeElem.STVORKA};
        LinkedList<ElemBase> arrElem = new LinkedList();
        LinkedList<E> outElem = new LinkedList();
        for (Map.Entry<eLayoutArea, ElemFrame> elemRama : getRoot().getHmElemFrame().entrySet()) {
        ...
        for (ElemBase elemBase : getRoot().getChildList()) { //первый уровень
            ...
            if (elemBase instanceof AreaSimple) {
                for (Map.Entry<eLayoutArea, ElemFrame> elemRama : ((AreaSimple) elemBase).getHmElemFrame().entrySet()) {
                    arrElem.add(elemRama.getValue());
                }
                for (ElemBase elemBase2 : elemBase.getChildList()) { //второй уровень
                    arrElem.add(elemBase2);
                    if (elemBase2 instanceof AreaSimple) {
                        for (Map.Entry<eLayoutArea, ElemFrame> elemRama : ((AreaSimple) elemBase2).getHmElemFrame().entrySet()) {
                            arrElem.add(elemRama.getValue());
                        }
                        for (ElemBase elemBase3 : elemBase2.getChildList()) { //третий уровень
                            arrElem.add(elemBase3);
                            if (elemBase3 instanceof AreaSimple) {
                                for (Map.Entry<eLayoutArea, ElemFrame> elemRama : ((AreaSimple) elemBase3).getHmElemFrame().entrySet()) {
                                    arrElem.add(elemRama.getValue());
                                }
                                for (ElemBase elemBase4 : elemBase3.getChildList()) { //четвёртый уровень
                                    arrElem.add(elemBase4);
                                    if (elemBase4 instanceof AreaSimple) {
                                        for (Map.Entry<eLayoutArea, ElemFrame> elemRama : ((AreaSimple) elemBase4).getHmElemFrame().entrySet()) {
                                            arrElem.add(elemRama.getValue());
                                        }
    ...
    public void drawWin(float scale, byte[] buffer, boolean line) {
        try {
            ...
            LinkedList<ElemGlass> elemGlassList = getElemList(eTypeElem.GLASS);
            elemGlassList.stream().forEach(el -> el.drawElemList());

            //Прорисовка импостов
            LinkedList<ElemImpost> elemImpostList = getElemList(eTypeElem.IMPOST);
            ...
            //Прорисовка створок
            LinkedList<AreaStvorka> elemStvorkaList = getElemList(eTypeElem.FULLSTVORKA);
            elemStvorkaList.stream().forEach(el -> el.drawElemList());

            if (line == true) {
                //Прорисовка размера
                this.drawLineLength();
                LinkedList<AreaSimple> areaList = getElemList(eTypeElem.AREA);
...
//        if (eTypeElem.ARCH == this.getTypeElem()) {
...
//        }
    ...
    public void print() {
        System.out.println(eTypeElem.AREA + " owner.id=" + owner.id + ", id=" + id + ", x1=" + x1 + ", y1=" + y1 + ", x2=" + x2 + ", y2=" + y2);
    }
}
