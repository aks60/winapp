...
public class Color2 {
    ...
    public static boolean colorFromProduct(Specification spc, Record detailRec) {  //см. http://help.profsegment.ru/?id=1107        
...
//                    colorFromArtdetID = colorFromArtikl(spc.artiklRec, side, colorFromTypesID, comp);
    ...
    private static int colorFromParam(Specification spc, int side, int colorFromTypesID) {
        try {
            int grup = spc.detailRec.getInt(COLOR_FK);
            List<Record> paramList = eParams.query().stream().filter(rec -> rec.getInt(eParams.grup) == grup && rec.getInt(eParams.numb) != 0).collect(toList());
            List<Record> artdetList = eArtdet.find(spc.artiklRec.getInt(eArtikl.id));
            ...
            for (Record artdetRec : artdetList) {
                ...
                if ((side == 1 && "1".equals(artdetRec.getStr(eArtdet.mark_c1)))
                    ...
                    if (artdetRec.getInt(eArtdet.color_fk) < 0) {
                        ...
                        //Одна текстура
                    } else if (artdetRec.getInt(eArtdet.color_fk) == colorFromTypesID) { //если есть такая текстура в ARTDET
    ...
    private static int colorFromTypes(Specification spc, int colorType) {
        try {
            switch (colorType) {
                ...
                case 9:
                    return spc.elem5e.iwin().colorID1; //по параметру (основа)
                case 4:
                    return spc.elem5e.iwin().colorID2; //по параметру (внутр.)                    
                case 12:
                    return spc.elem5e.iwin().colorID3; //по параметру (внешн.) 
                default:
