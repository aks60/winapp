...
public enum eFurndet implements Field {
    ...
    id2("4", "10", "1", "Ссылка на параметры и на зависимую/вложенную спецификацию фурнитуры:", "FINCB"), //TODO Изменить бред 
    fincs("4", "10", "1", "ID зависимого фурнитурного набора из  FINCB  по данной фурнитуре", "FINCS"), //furndet_id  
    level("5", "5", "1", "Уровень вложения", "FLEVE"), //(1 - основная, 2 - зависимая, 3 - вложенная)

    types("5", "5", "1", "Подбор текстуры", "CTYPE"), //(CTYPE) 0 - указана, 273 - на основе изделия, 546 - по внутр. изделия, 799 - по заполнению (зависимая?) 801 - по основе изделия, 819 - по внешн. изделия, 1092 - по параметру (внутр.), 1638 - по основе в серии, 1911 - по внутр. в серии, 2184 - по внешн. в серии, 3003 - по профилю, 3145 - по параметру (основа), 3276 - по параметру (внешн.), 4095 - по заполнению.    
    color_fk("4", "10", "1", "Текстура", "CLNUM"), //(CLNUM)
    artikl_id("4", "10", "1", "Артикл", "artikl_id"), //(ANUMB)
    furniture_id("4", "10", "0", "Фурнитура", "furniture_id"); //(FUNIC)
