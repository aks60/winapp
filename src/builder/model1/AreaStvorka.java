...
public class AreaStvorka extends AreaSimple implements IStvorka {
    ...
    public AreaStvorka(Wincalc winc, IArea5e owner, GsonElem gson) {
        ...
        ElemFrame stvBot = (eProp.old.read().equals("0")) 
                ? new builder.model1.ElemFrame(this, gson.id() + .1f, Layout.BOTT, gson.param().getAsJsonObject(PKjson.stvorkaBottom), gson)
                : new builder.model1.ElemFrame(this, gson.id() + .1f, Layout.BOTT, gson.param().getAsJsonObject(PKjson.stvorkaBottom), gson);
        frames.put(stvBot.layout(), stvBot);
        ElemFrame stvRigh = (eProp.old.read().equals("0")) 
                ? new builder.model1.ElemFrame(this, gson.id() + .2f, Layout.RIGHT, gson.param().getAsJsonObject(PKjson.stvorkaRight), gson)
                : new builder.model1.ElemFrame(this, gson.id() + .2f, Layout.RIGHT, gson.param().getAsJsonObject(PKjson.stvorkaRight), gson);
        frames.put(stvRigh.layout(), stvRigh);
        ElemFrame stvTop = (eProp.old.read().equals("0")) 
                ? new builder.model1.ElemFrame(this, gson.id() + .3f, Layout.TOP, gson.param().getAsJsonObject(PKjson.stvorkaTop), gson)
                : new builder.model1.ElemFrame(this, gson.id() + .3f, Layout.TOP, gson.param().getAsJsonObject(PKjson.stvorkaTop), gson);
        frames.put(stvTop.layout(), stvTop);
        ElemFrame stvLeft = (eProp.old.read().equals("0")) 
                ? new builder.model1.ElemFrame(this, gson.id() + .4f, Layout.LEFT, gson.param().getAsJsonObject(PKjson.stvorkaLeft), gson)
                : new builder.model1.ElemFrame(this, gson.id() + .4f, Layout.LEFT, gson.param().getAsJsonObject(PKjson.stvorkaLeft), gson);
