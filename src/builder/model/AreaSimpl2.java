...
public class AreaSimpl2 implements IArea5e {
    ...
    public AreaSimpl2(Wincalc winc) {
    ...
    public AreaSimpl2(Wincalc winc, AreaSimpl2 owner, GsonElem gson, float width, float height) {
    ...
    public AreaSimpl2(Wincalc winc, AreaSimpl2 owner, GsonElem gson, float width, float height, Form form) {
        super(gson.id(), winc, owner, gson);
        this.form = form;
        this.layout = gson.layout();
        this.colorID1 = winc.rootGson.color1();
        ...
        if (owner != null) {
            if (owner.childs.isEmpty() == true) { //если childs.isEmpty то prevArea искать нет смысла
                ...
                for (int index = owner.childs.size() - 1; index >= 0; --index) { //т.к. this area ёщё не создана начнём с конца
                    if (owner.childs.get(index) instanceof AreaSimpl2) {
                        AreaSimpl2 prevArea = (AreaSimpl2) owner.childs.get(index);
        ...
        try {
            if (id() == 0) {
                ...
                if (k != 1) {
                    ...
                    for (AreaSimpl2 e : winc.listArea) { //перебор всех вертикальных area
                ...
                if (k != 1) {
                    ...
                    for (Com5t e : childs) { //изменение детей по ширине
                        if (e.owner.layout == Layout.HORIZ && (e.type() == Type.AREA || e.type() == Type.STVORKA)) {
                            ((AreaSimpl2) e).resizeY(k * e.lengthY()); //рекурсия изменения детей

                        } else {
                            if (e instanceof AreaSimpl2) {
                                for (Com5t e2 : ((AreaSimpl2) e).childs) {
                                    if (e2.owner.layout == Layout.HORIZ && (e2.type() == Type.AREA || e2.type() == Type.STVORKA)) {
                                        ((AreaSimpl2) e2).resizeY(k * e2.lengthY()); //рекурсия изменения детей
                                    }
                                }
                            }
                        }
            ...
            if (id() == 0) {
                ...
                if (k != 1) {                   
                    ...
                    for (Com5t e : childs) { //изменение детей по высоте
                        if (e.owner.layout == Layout.HORIZ && (e.type() == Type.AREA || e.type() == Type.STVORKA)) {
                            ((AreaSimpl2) e).resizeX(k * e.lengthX()); //рекурсия изменения детей

                        } else {
                            if (e instanceof AreaSimpl2) {
                                for (Com5t e2 : ((AreaSimpl2) e).childs) {
                                    if (e2.owner.layout == Layout.HORIZ && (e2.type() == Type.AREA || e2.type() == Type.STVORKA)) {
                                        ((AreaSimpl2) e2).resizeX(k * e2.lengthX()); //рекурсия изменения детей
                                    }
                                }
                            }
                        }
