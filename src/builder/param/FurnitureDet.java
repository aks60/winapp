        ...
        try {
            switch (grup) {
                ...
                case 24004: //Если створка прилегает к артикулу 
                    if (areaStv.frames().entrySet().stream().filter(el -> winc.mapJoin.get(el.getValue(), 2).artiklRec().getStr(eArtikl.code).equals(rec.getStr(TEXT))).findFirst().orElse(null) == null) {
                        return false;
                    }
                    break;
                ...
                case 25033: //Фурнитура штульповая 
                {
                    if (rec.getStr(TEXT).equals("Да")) {
                        ...
                        for (Map.Entry<Layout, IElem5e> entry : areaStv.frames().entrySet()) {
                            if (winc.mapJoin.get(entry.getValue(), 2).type() == Type.SHTULP) {
                                ret = true;
                            }
                        }
                        if (ret == false) {
