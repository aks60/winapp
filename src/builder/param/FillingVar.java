...
public class FillingVar extends Par5s {
    ...
    public boolean filter2(ElemSimple elem5e, Record glasgrpRec) {
    ...
    public boolean check2(ElemSimple elemGlass, Record rec) {

        int grup = rec.getInt(GRUP);
        try {
            switch (grup) {
                case 13001:  //Если признак состава 
                    if (UPar.is_11001_11002_12001_12002_13001_14001_15001_33001_34001(rec.getStr(TEXT), elemGlass) == false) {
                        return false;
                    }
                    break;
                case 13003:  //Тип проема 
                    if (!UPar.is_13003_14005_15005_37008(rec.getStr(TEXT), elemGlass)) {
                        return false;
                    }
                    break;
                case 13005: //Заполнение типа 
                    if ("Стекло".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 1) {
                        return false;
                    } else if ("Стеклопакет".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 2) {
                        return false;
                    } else if ("Сендвич".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 3) {
                        return false;
                    } else if ("Вагонка".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 4) {
                        return false;
                    } else if ("Алюминевый лист".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 5) {
                        return false;
                    } else if ("Специальное стекло".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 6) {
                        return false;
                    } else if ("Конструктив".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 9) {
                        return false;
                    } else if ("Панель откоса".equals(rec.getStr(TEXT)) && elemGlass.artiklRecAn.getInt(eArtikl.level2) != 15) {
                        return false;
                    }
                    break;
                case 13014:  //Углы ориентации стороны, ° 
                    elemGlass.spcRec.mapParam.put(grup, rec.getStr(TEXT));
                    break;
                case 13015:  //Форма заполнения 
                    //"Прямоугольное", "Не прямоугольное", "Не арочное", "Арочное" (TypeElem.AREA - глухарь)
                    if ("Прямоугольное".equals(rec.getStr(TEXT)) && Type.RECTANGL.equals(elemGlass.owner().type()) == false
                            && Type.AREA.equals(elemGlass.owner().type()) == false && Type.STVORKA.equals(elemGlass.owner().type()) == false) {
                        return false;
                    } else if ("Не прямоугольное".equals(rec.getStr(TEXT)) && (Type.TRAPEZE.equals(elemGlass.owner().type()) == false
                            && Type.TRIANGL.equals(elemGlass.owner().type()) == false)) {
                        return false;
                    } else if ("Арочное".equals(rec.getStr(TEXT)) && Type.ARCH.equals(elemGlass.owner().type()) == false) {
                        return false;
                    } else if ("Не арочное".equals(rec.getStr(TEXT)) && Type.ARCH.equals(elemGlass.owner().type()) == true) {
                        return false;
                    }
                    break;
                ...
                case 13081:  //Для внешнего/внутреннего угла плоскости, ° или Мин. внутр. угол плоскости, ° 
                    if ("ps3".equals(eSetting.find(2))) {
                        if (elemGlass.anglFlat[0] > rec.getFloat(TEXT)) {
                            return false;
                        }
                    } else if (UPar.is_13081_13082_13086_13087(elemGlass, rec.getStr(TEXT))) {
                        return false;
                    }
                case 13082:  //Макс. внутр. угол плоскости, °
                    if ("ps3".equals(eSetting.find(2))) {
                        if (elemGlass.anglFlat[1] > rec.getFloat(TEXT)) {
                            return false;
                        }
                    }
                case 13086:  //Мин. внешний угол плоскости, °
                    if ("ps3".equals(eSetting.find(2))) {
                        if (elemGlass.anglFlat[2] > rec.getFloat(TEXT)) {
                            return false;
                        }
                    }
                case 13087:  //Макс. внешний угол плоскости, °
                    if ("ps3".equals(eSetting.find(2))) {
                        if (elemGlass.anglFlat[3] > rec.getFloat(TEXT)) {
                            return false;
                        }
                    }
